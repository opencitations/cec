...
class TEIXMLtoRDFConverter:
    ...
    def __init__(self, xml_file, output_jsonld_file, sections_mapping_file=None):
        self.xml_file = xml_file
        self.output_jsonld_file = output_jsonld_file
    ...
    def convert_to_rdf(self):
        ...
        for div in root.findall(".//tei:div", namespaces=ns):
            ...
            if head is not None:
                head_text = head.text.strip() if head.text else ""
                if self.aligned_sections and head_text in self.aligned_sections:
                    last_head = self.aligned_sections[head_text]
                else:
                    if head_n_attribute > head_no_n_attribute:
                        if has_n_attribute:
                            if 'n' in head.attrib:
                ...
                if refs:
                    ...
                    de.create_section()
                    if type(last_head) is list:
                        last_head = "_".join(last_head)
        ...
        my_graph_storer.store_graphs_in_file(self.output_jsonld_file)
...
class PDFProcessor:
    ...
    def process_pdf(self, align_headings=False):
        ...
        mapping_output = self.output_json_path + os.path.sep + os.path.basename(input_pdf_path[0]).split(".pdf")[
            0] + "_section_mapping" + ".json"
        if align_headings:
            # implement sections' headings alignment
            run(output_json_name, ["Introduction", "Related Works", "Methods", "Materials", "Results", "Discussion",
                                   "Conclusion"],
                json_output=output_json_name, mapping_output=mapping_output,
                predefined_mappings_file=PREDEFINED_MAPPINGS_PATH)

        output_jsonld_name = (self.output_json_path + os.path.sep + os.path.basename(input_pdf_path[0]).split(".pdf")[0] +
                              ".jsonld")
        if align_headings:
            tei_to_rdf_converter = TEIXMLtoRDFConverter(xml_file=xml_file_path, output_jsonld_file=output_jsonld_name, sections_mapping_file=mapping_output)
        else:
            tei_to_rdf_converter = TEIXMLtoRDFConverter(xml_file=xml_file_path, output_jsonld_file=output_jsonld_name)
